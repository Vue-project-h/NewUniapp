...
<script>
	...
	export default {
		...
		methods: {
		    ...
		    startReg() {
				//注册
				let re=/^[a-zA-Z0-9\u4E00-\u9FA5_-]{3,20}$/;
				let ps=/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[a-zA-Z\d]{6,20}$/g;
				if(this.isRotate){
					//判断是否加载中，避免重复点击请求
					return false;
				}
				...
				if (!re.test(this.nameData)) {
				    uni.showToast({
				        icon: 'none',
						position: 'bottom',
				        title: '请输入3-20位数字、字母或下划线'
				    });
				    return false;
				}
		        if (!ps.test(this.passData)) {
		            uni.showToast({
		                icon: 'none',
						position: 'bottom',
		                title: '密码必须有大小写字母和数字且6-20位！'
		            });
		            return false;
		        }
				if (this.ConfirmData!=this.passData) {
				    uni.showToast({
				        icon: 'none',
						position: 'bottom',
				        title: '两次输入的密码不正确！'
