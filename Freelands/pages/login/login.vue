<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="nameData" type="text" maxlength="11" placeholder="用户名"></wInput>
	...
	export default {
		...
		methods: {
			...
			startLogin() {
				...
				// },3000)
				uni.request({
					url: this.base_url + 'users/logins',
					method: 'POST',
					data: {
						name: this.nameData,
					...
					success: res => {
						if (res.data.code == 1) {
							...
							for (var attr in res.data.data.userinfo) {
								console.log(attr)
								getApp().globalData.userinfo[attr] = res.data.data.userinfo[attr]
							}
							// let myinfo = uni.getStorage({ //【2】在应用初始化的时候，从本地缓存取出用户信息
							//             key:'userinfo',
							//             complete: (res) => {
							// 				console.log(res)
							//             }
